            ...
            <div className="input-container">
                <input
                    ...
                    onBlur={(e) => {
                        if (e.target.value !== "")
                            onAmount(
                                Math.min(maxAmount, Math.max(minAmount, parseInt(e.target.value)))
                            )
                    }}
                    placeholder={`${formatLoan(minAmount)}`}
                    className="loan-input w-full  rounded-md border-0 py-3 px-5 text-gray-900 ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-0 font-bold text-xl leading-7"
                    required
                    min={minAmount}
                    max={maxAmount}
                />
